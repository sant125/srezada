# The Site Reliability Workbook, destrinchado

Notas de estudo do [Google SRE Workbook](https://sre.google/workbook/table-of-contents/), capítulo a capítulo. Não é resumo: é o livro reexplicado até eu entender de verdade, com analogia antes de fórmula e jargão só depois da ideia.

## Formato de cada capítulo

- A ideia central primeiro, o nome técnico depois
- Analogias e exemplos numéricos antes de definição formal
- Conexão com meu stack (Kubernetes, Prometheus, Go, AWS/GCP)
- O que isso vira em entrevista

## Índice

**Parte I: Foundations**

- [x] [01 · How SRE Relates to DevOps](01-how-sre-relates-to-devops.md)
- [x] [02 · Implementing SLOs](02-implementing-slos.md)
- [ ] 03 · SLO Engineering Case Studies
- [x] [04 · Monitoring](04-monitoring.md)
- [ ] 05 · Alerting on SLOs
- [ ] 06 · Eliminating Toil
- [ ] 07 · Simplicity

**Parte II: Practices**

- [ ] 08 · On-Call
- [ ] 09 · Incident Response
- [ ] 10 · Postmortem Culture: Learning from Failure
- [ ] 11 · Managing Load
- [ ] 12 · Introducing Non-Abstract Large System Design
- [ ] 13 · Data Processing Pipelines
- [ ] 14 · Configuration Design and Best Practices
- [ ] 15 · Configuration Specifics
- [ ] 16 · Canarying Releases

**Parte III: Processes**

- [ ] 17 · Identifying and Recovering from Overload
- [ ] 18 · SRE Engagement Model
- [ ] 19 · SRE: Reaching Beyond Your Walls
- [ ] 20 · Organizational Change Management in SRE
